...
class Num(Interpretation):
	def __init__(self, i):
		self.i = Precision.decimal(i, 10)
		
	def __str__(self):
		return str(Precision.num(Precision.decimal(self.i, 5)))
	...
	def solve(self, vars):
		...
		if type(i2) is Mat:
			if type(self.i1) is Rows:
				...
				elif type(i1) is Range:
					m = i2.i.get_row((i1.i1, Precision.num(i1.i2) + 1))
					if m is None:
						return Err("out of bounds")
				...
				if is_integer(i1):
					...
					m = i2.i.get_column((i1.i1, Precision.num(i1.i2) + 1))
					if m is None:
						return Err("out of bounds")
...
class Factorial(Interpretation):
	...
	def solve(self, vars):
		...
		return Num(math.factorial(Precision.num(i)))
